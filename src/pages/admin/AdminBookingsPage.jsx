...
export default function AdminBookingsPage() {
  ...
  const handleCancelBooking = async () => {
    if (!drawer) return;
    if (!window.confirm(`Cancel booking ${drawer.bookingReference}? This cannot be undone.`)) return;
    setCancellingBooking(true);
    try {
      const r = await adminApi.patch(`/bookings/${drawer._id}`, { status: 'cancelled', reason: 'Cancelled by admin' });
      const updated = r.data.data?.booking;
          ...
          <>
            ...
            {drawer.status !== 'cancelled' && (
              <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 space-y-2">
                {drawer.payment?.status === 'completed' && (
                  <button
                ...
                <button
                  ...
                  className="w-full border border-red-300 text-red-600 rounded-xl py-2.5 font-semibold hover:bg-red-50 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {cancellingBooking ? (
                    <><AlertCircle className="w-4 h-4 animate-pulse" /> Cancelling...</>
                  ) : (
